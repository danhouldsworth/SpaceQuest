...
Particle.prototype.collide      = function(that){
    ...
    if (interaction.near(this, that) && interaction.touching()){
        ...
        if (interaction.seperation < 0.99 * interaction.size){ // Try and save some cycles
            ...
            var thatAcc = Math.max(1, antiSqueezeForce * this.mass / (this.mass + that.mass));
            var thisAcc = Math.max(1, antiSqueezeForce - thatAcc);
...
Ship.prototype.spinThrustExhaust = function(direction){
    var thrustParticle1 = new Thrust(
        ...
        this.y - direction * 0.95 * this.size * Math.cos(this.angle + direction * 1.3),
        +direction * this.thrustSpeed * 0.5 * Math.sin(this.angle),
        -direction * this.thrustSpeed * 0.5 * Math.cos(this.angle),
        this.thrustSize * 0.3,
        this
    );
    var thrustParticle2 = new Thrust(
        this.x - direction * 0.95* this.size * Math.sin(this.angle + direction * 1.3),
        this.y + direction * 0.95* this.size * Math.cos(this.angle + direction * 1.3),
        -direction * this.thrustSpeed * 0.5 * Math.sin(this.angle),
        +direction * this.thrustSpeed * 0.5 * Math.cos(this.angle),
    ...
    gameObjects.push(thrustParticle1);
    gameObjects.push(thrustParticle2);
    return 2 * thrustParticle1.mass * this.thrustSpeed;
};
Ship.prototype.mainThrustExhaust = function(){
    var thrustParticle = new Thrust(
        this.x - (this.size + this.thrustSize) * Math.cos(this.angle),
        this.y - (this.size + this.thrustSize) * Math.sin(this.angle),
        -this.thrustSpeed * Math.cos(this.angle),
        -this.thrustSpeed * Math.sin(this.angle),
        this.thrustSize,
        this
    );
    gameObjects.push(thrustParticle);
    return thrustParticle.mass * this.thrustSpeed;
};
Ship.prototype.fireGun          = function(deltaT){
    var bulletSpeed = 1;
    var bulletMass  = Math.max(this.size / 4 - 1, 2);
    gameObjects.push(new Bullet(
        this.x + 1.8 * this.size * Math.cos(this.angle) + .1 * Math.random() - 2,
        this.y + 1.8 * this.size * Math.sin(this.angle) + .1 * Math.random() - 2,
...
Ship.prototype.fireCanonBall   = function(){
    ...
    var cannonBall = new Fireball(
        this.x - 2.0 * this.size * Math.cos(this.angle),
        this.y - 2.0 * this.size * Math.sin(this.angle),
...
Ship.prototype.getPilotCommand  = function(deltaT){
    ...
    this.ax = this.ay = this.spinDot = 0;
    if (keyState[playerKeys.left[   this.player]])      {this.thrustLeft    = true;}    else {this.thrustLeft   = false;}
    if (keyState[playerKeys.right[  this.player]])      {this.thrustRight   = true;}    else {this.thrustRight  = false;}
    if (keyState[playerKeys.thrust[ this.player]])      {this.thrusting     = true;}    else {this.thrusting    = false;}
    if (keyState[playerKeys.fire[   this.player]])      {this.firing        = true;}    else {this.firing       = false;}
